...
//     $this->app->bind('finexus', fn() => new \Jiannius\Finexus\Finexus);
...
class Finexus
{
    ...
    public function checkout($parameters = []) : mixed
    {
        ...
        return redirect(env('FINEXUS_URL').'?'.$paymentMessage);
    }
    ...
    public function query($data) : mixed
    {
        ...
        $paymentRequestEntity->setter('MerchantID', data_get($data, 'MerchantID'));
        $paymentRequestEntity->setter('ServiceID', data_get($data, 'ServiceID'));
        $paymentRequestEntity->setter('MerchRefNo', data_get($data, 'MerchRefNo'));
    ...
    public function getConfigFile() : string
    {
        if (!Storage::exists($this->configFilePath)) {
            $configs = [
                'VersionNo' => '06',
                'ServiceID' => 'FNX',
                'MerchantID' => env('FINEXUS_MERCHANT_ID'),
                'TxnChannel' => 'WEB',
                'LangLocale' => 'en',
    ...
    public function test() : bool
    {
        ...
        return data_get($query, 'QueryStatus') === '12'; // status 12 = transaction not found
    }
    ...
    {
        $this->currency = data_get($this->currencyCodes, $code);
    ...
    {
        $code = data_get($response, 'TxnStatus');
